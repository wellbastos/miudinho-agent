...
func TestSLOPolicyReconcileDiscoversServicesByLabels(t *testing.T) {
	...
	slo := &sre.SLOPolicy{
		ObjectMeta: metav1.ObjectMeta{Name: "global-slo"},
...
func TestSLOPolicyReconcileGlobalExcludesNamespaces(t *testing.T) {
	scheme := runtime.NewScheme()
	mustAddScheme(t, scheme)
	...
	slo := &sre.SLOPolicy{
		...
		Spec: sre.SLOPolicySpec{
			...
			ExcludedNamespaces: []string{"kube-system", "o11y"},
			Signals:            sloSignals(1.0, 0.0, 0.1),
			ScheduleSeconds:    120,
		},
	}
	checkout := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "checkout",
			Namespace: "apps",
			Labels:    map[string]string{"miudinho.o11y.io/enabled": "true"},
		},
	}
	coredns := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "coredns",
	...
	cl := fake.NewClientBuilder().
		WithScheme(scheme).
		WithStatusSubresource(&sre.SLOPolicy{}, &sre.PredictiveIncident{}).
		WithObjects(slo, checkout, coredns).
		Build()
	...
	var incidents sre.PredictiveIncidentList
	if err := cl.List(context.Background(), &incidents); err != nil {
		t.Fatalf("List returned error: %v", err)
	}
	if len(incidents.Items) != 1 {
		t.Fatalf("expected 1 predictive incident, got %d", len(incidents.Items))
	}
	if incidents.Items[0].Namespace != "apps" {
		t.Fatalf("expected apps incident, got namespace %q", incidents.Items[0].Namespace)
	}
}
